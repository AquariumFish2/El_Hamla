...
namespace El_Hamla
{
    public partial class AA_pro_add : Form
    {
        DataTable dt_MO;
        DataTable dt_car;
        DataTable dt_max;
        ...
        private void AA_pro_add_Load(object sender, EventArgs e)
        {
            ...
            string query = "SELECT MAX(ProjectID) FROM AA_Projects";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    // إذا كانت النتيجة null يتم تعيينها إلى 0 وإضافة 1
                    int nextProjectID = (result != DBNull.Value) ? Convert.ToInt32(result) + 1 : 1;

                    // عرض القيمة الجديدة في TextBox
                    textBox3.Text = nextProjectID.ToString();
                }
                catch (Exception ex)
                {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                cLS.insertProject(Convert.ToInt32(textBox3.Text), textBox1.Text, Convert.ToDecimal(textBox4.Text), Convert.ToDecimal(textBox6.Text), Convert.ToDecimal(textBox5.Text), Convert.ToDecimal(textBox7.Text));
                cL.insert_MO_table(Convert.ToInt32(textBox10.Text), textBox1.Text, "خزينة مشروع", 0);
                MessageBox.Show("تمت إضافة المشروع وفتح حساب له ");
                this.Close();
            }
            catch
            {
